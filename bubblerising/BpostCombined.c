...
int WriteFileAll(char *BaseName, double tc, int CellNO, char **VarName, double **xyDeltaVars, int VarNO);
int WriteFileAllChains(char *BaseName, double tc, char **VarName, double **xyDeltaVars, int VarNO, int **ChainLoopIndex);
int WriteFileMaxChain(char *BaseName, double tc, char **VarName, double **xyDeltaVars, int VarNO, int **ChainLoopIndex);
...
int JetRoots(int iteration, double tc, scalar kappa, scalar omega, char writeNBinfo);
int JetRecognition(int iteration, double tc, double **xyDKOjet, int CellNO, int **JetRoots);

int findlocalmaxminIndependent(double *var, int NO, int *index, char *type, int *noindex);

int FilteringVar(double *Var, int CellNO, double *VarF, char smoothing_lowpass, int filterlevels, int *filterrepeats, int *filterpoints);
int smoothing(double *varin, int NO, int order, double *varout);
...
double Time_BGN = 0.0, Time_STP = 0.001, Time_END = 0.1;
...
int main(int argc, char **argv)
{
	...
	case 4:
	{
		Time_BGN = atof(argv[1]);
		Time_STP = atof(argv[2]);
		Time_END = atof(argv[3]);
		break;
	}
	default:
	{
...
{
	int iloop;
	char name[500], nameloadfile[500];
	double tc;
	scalar alpha[], kappa[], omega[], DUMMY[], ux[], uy[];
	vector vn[];
	;
	for(tc = Time_BGN, iloop = 0; tc <= Time_END + 0.00001; tc += Time_STP, iloop++)
	{
		// loading
		;
		...
		// Check the interface
		;
//		JetRoots(iloop, tc, kappa, omega, 'n');
		;
		// bview output
		;
		sprintf(name, "bview-f-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		draw_vof ("f", linear = true, lw = 2.0);
		squares ("f", min = 0, max = 1, linear = true, spread = 21);
		box(lw = 1.0); // axis
		//cells(lw = 0.50); // grid
		save (name);
		;
		sprintf(name, "bview-ux-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		draw_vof ("f", linear = true, lw = 2.0);
		squares ("u.x", linear = true, spread = 21);
		box(lw = 1.0); // axis
		//cells(lw = 0.50); // grid
		save (name);
		;
		sprintf(name, "bview-uy-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		draw_vof ("f", linear = true, lw = 2.0);
		squares ("u.y", linear = true, spread = 21);
		box(lw = 1.0); // axis
		//cells(lw = 0.50); // grid
		save (name);
		;
		sprintf(name, "bview-omega-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		draw_vof ("f", linear = true, lw = 2.0);
		squares ("omega", linear = true, spread = 21);
		box(lw = 1.0); // axis
		//cells(lw = 0.50); // grid
		save (name);
		;
		sprintf(name, "bview-pressure-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		draw_vof ("f", linear = true, lw = 2.0);
		squares ("pressure", linear = true, spread = 21);
		box(lw = 1.0); // axis
		//cells(lw = 0.50); // grid
		save (name);
		;
		sprintf(name, "bview-grid-%.4f.png", tc);
		clear();
		view (width = 1000, height = 1000, ty = -0.50, sx = 1.0, sy = 1.0, quat = {0, 0, -0.707, 0.707});
		box(lw = 1.0); // axis
		cells(lw = 0.50); // grid
		save (name);
		;
/*		view (width = 3200, height = 1600, ty = -0.50, quat = {0, 0, -0.707, 0.707});
		clear();
		draw_vof ("f", lw = 1);
		squares ("f", linear = true, min = 0.0, max = 1.0);
		box (notics = true);
		mirror ({-1})
		{
			draw_vof ("f", lw = 1);
			squares ("DUMMY", linear = true);
			box (notics = true);
		}
		draw_string("TESTME");
		save (name);
*/		;
		...
		onlyf[3] = 'n';
		onlyf[4] = 'n';
		output_tecplot2D_nodal(name, tc, {ux, uy, pressure, omega}, onlyf, 1);
		;
		sprintf(name, "out-F1-%.4f.plt", tc);
		output_tecplot2D_Intrfc(name, f, tc, 1, iloop);
		;
		sprintf(name, "out-F2-%.4f.plt", tc);
		output_tecplot2D_Intrfc(name, fdrop, tc, 1, iloop);
		;
/*		// type-> r: reqular, l: in liquid only, s: second interface
		sprintf(name, "out-AllTime-F2.plt");
		output_tecplot2D_AllTime(name, tc, iloop, 's', 1, f, fdrop, DUMMY, 1);
		;
		sprintf(name, "out-AllTime-OMEGA.plt");
		output_tecplot2D_AllTime(name, tc, iloop, 'l', 1, f, omega, DUMMY, 1);
		;
		sprintf(name, "out-AllTime-PRESSURE.plt");
		output_tecplot2D_AllTime(name, tc, iloop, 'r', 1, f, pressure, DUMMY, 1);
		;
		sprintf(name, "out-AllTime-VELOCITY.plt");
		output_tecplot2D_AllTime(name, tc, iloop, 'l', 2, f, u.x, u.y, 1);
*/		;
/*		sprintf(name, "out-CC-%.4f.plt", tc);
		output_tecplot2D_cellcenter(name, tc, kappa, omega, pressure);
		;
		sprintf(name, "out-IC-%.4f.plt", tc);
		output_tecplot2D_interfacecells(name, tc, kappa, omega, pressure);
*/		;
/*		char inter[80];
		sprintf(inter, "interface-%.3f.txt",tc);
		FILE* fp42 = fopen(inter,"w");
		output_facets (f,fp42);
		fclose (fp42);
		char mesh[80];
		sprintf(mesh, "mesh-%.3f.txt", tc);
		FILE* fp43 = fopen(mesh, "w");
		output_cells(fp43);
		fclose (fp43);
*/		;
...
int JetRoots(int iteration, double tc, scalar kappa, scalar omega, char writeNBinfo)
{
	...
	int *IndexTotalLeaf, **ChainLoopIndex, **JetRoots; // JetRoots for different methods
	...
	JetRoots = (int **)calloc(interfaceNO, sizeof(int *));
	for (i = 0; i < interfaceNO; i++)
	{
		ChainLoopIndex[i] = (int *)calloc(interfaceNO, sizeof(int));
		JetRoots[i] = (int *)calloc(2, sizeof(int));
	}
	...
	// find jet roots
	;
	JetRecognition(iteration, tc, xyDKOjet, MaxChainCellNO, JetRoots);
	...
	case 'Y':
	{
		strcpy(VarFileName[0], "x");
		strcpy(VarFileName[1], "y");
		strcpy(VarFileName[2], "Delta");
		strcpy(VarFileName[3], "Kappa");
		strcpy(VarFileName[4], "Omega");
		WriteFileAll("out_all_", tc, interfaceNO, VarFileName, xyDKO, VarNO);
		WriteFileAllChains("out_AllChains_", tc, VarFileName, xyDKO, VarNO, ChainLoopIndex);
		WriteFileMaxChain("out_MaxChain_", tc, VarFileName, xyDKO, VarNO, ChainLoopIndex);
		break;
	}
	}
	WriteTmprlData("OutTemporal-1.plt", iteration, tc, xyDKO, JetRoots[1]);
	WriteTmprlData("OutTemporal-2.plt", iteration, tc, xyDKO, JetRoots[2]);
	WriteTmprlData("OutTemporal-3.plt", iteration, tc, xyDKO, JetRoots[3]);
	;
...
int JetRecognition(int iteration, double tc, double **xyDKOjet, int CellNO, int **JetRoots)
{
	...
	filterpoints[5] = 2;
	filterpoints[6] = 1;
	FilteringVar(xyDKOjet[3], CellNO, kppF, 's', 7, filterrepeats, filterpoints);
	FilteringVar(xyDKOjet[4], CellNO, omgF, 's', 7, filterrepeats, filterpoints);
	;
	// find local maximums
	;
	...
	// writing files
	;
	WriteExtremums("OutExtremums.plt", iteration, tc, xyDKOjet, kppF, omgF, CellNO, Kindex, KindexNO, Oindex, OindexNO, JetRoots);
	Write2DJetRoot("Out2DJetRoot.plt", iteration, tc, xyDKOjet, kppF, omgF, CellNO, Kindex, KindexNO, Oindex, OindexNO, JetRoots);
	;
	// free memory
	;
	free(kppF);
...
int Write2DJetRoot(char *name, int Iteration, double tc, double **xyDKO, double *kppF, double *omgF, int CellNO, int *Kindex, int KindexNO, int *Oindex, int OindexNO, int **JetRoots)
{
	...
	fprintf(fp, "\r\nZONE T=\"AllData[%.4f]\"\r\n"
		"N = %d, E = %d, DATAPACKING = BLOCK, ZONETYPE = FEQUADRILATERAL", tc, CellNO * 4, CellNO);
	...
	for (f = 1; f <= JetRoots[0][0]; f++)
	{
		...
		fprintf(fp, "variables = x, y, Delta, Kappa, Omega, KappaFiltered, OmegaFiltered");
		fprintf(fp, "\r\nZONE T=\"JetRoot[%d][%.4f]\"\r\n"
			"N = %d, E = %d, DATAPACKING = BLOCK, ZONETYPE = FEQUADRILATERAL", f, tc, 2 * 4, 2);
		fprintf(fp, "\r\nVARLOCATION = (NODAL, NODAL, CELLCENTERED, CELLCENTERED, CELLCENTERED, CELLCENTERED, CELLCENTERED)");
		fprintf(fp, "\r\nSOLUTIONTIME = %.4e", tc);
...
int FilteringVar(double *Var, int CellNO, double *VarF, char smoothing_lowpass, int filterlevels, int *filterrepeats, int *filterpoints)
{
	...
	case 'S':
	{
		for (l = 0; l < filterlevels; l++)
		{
			...
			{
				smoothing(VarI, CellNO, filterpoints[l], VarO);
				smoothing(VarO, CellNO, filterpoints[l], VarI);
			}
		}
		break;
	}
...
int FindAllChainLoops(double tc, int interfaceNO, const double deltaMIN, double **xyDelta, int **ChainLoopIndex, char writeNBinfo)
{
	...
	for (k = 1; k <= ChainLoopIndex[0][0]; k++)
	{
		if (xyDelta[0][ChainLoopIndex[k][1]] < xyDelta[0][ChainLoopIndex[k][ChainLoopIndex[k][0]]])
		{
...
int WriteFileAll(char *BaseName, double tc, int CellNO, char **VarName, double **xyDeltaVars, int VarNO)
{
	...
	strcat(name, ctmp);
	fp = fopen(name, "w");
	strcpy(name, "variables = \"x\", \"y\"");
	for (k = 2; k < VarNO; k++)
	{
		sprintf(ctmp, ", \"%s\"", VarName[k]);
		strcat(name, ctmp);
	}
	fprintf(fp, "%s", name);
	fprintf(fp, "\r\nZONE T=\"tc[%.3f]\"\r\nN = %d, E = %d, DATAPACKING = BLOCK, ZONETYPE = FEQUADRILATERAL", tc, CellNO * 4, CellNO);
	...
	for (i = 0; i < CellNO; i++)
	{
		fprintf(fp, "\r\n%e", xyDeltaVars[0][i] - xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[0][i] + xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[0][i] + xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[0][i] - xyDeltaVars[2][i]);
	}
	for (i = 0; i < CellNO; i++)
	{
		fprintf(fp, "\r\n%e", xyDeltaVars[1][i] - xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[1][i] - xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[1][i] + xyDeltaVars[2][i]);
		fprintf(fp, "\r\n%e", xyDeltaVars[1][i] + xyDeltaVars[2][i]);
	}
...
int WriteFileMaxChain(char *BaseName, double tc, char **VarName, double **xyDeltaVars, int VarNO, int **ChainLoopIndex)
{
	...
	for (k = 1; k <= ChainLoopIndex[0][0]; k++)
	{
		if (CellNO < ChainLoopIndex[k][0])
		{
			CellNO = ChainLoopIndex[k][0];
			MaxChain = k;
		}
	}
	strcpy(name, BaseName);
...
int lowpassfilter(double *varin, int NO, int order, double *varout)
{
	...
	case 6:
	{
		...
		;
		for (i = 3; i < NO - 4; i++)
			varout[i] = dr6th[1] * varin[i - 3] + dr6th[2] * varin[i - 2] + dr6th[3] * varin[i - 1]
			+ dr6th[4] * varin[i] + dr6th[5] * varin[i + 1] + dr6th[6] * varin[i + 2] + dr6th[7] * varin[i + 3];
		break;
	}
	case 8:
	{
		...
		;
		for (i = 4; i < NO - 5; i++)
			varout[i] = dr8th[0] * varin[i - 4] + dr8th[1] * varin[i - 3] + dr8th[2] * varin[i - 2] + dr8th[3] * varin[i - 1]
			+ dr8th[4] * varin[i] + dr8th[5] * varin[i + 1] + dr8th[6] * varin[i + 2] + dr8th[7] * varin[i + 3] + dr8th[8] * varin[i + 4];
		break;
	}
	}
	return 1;
...
int WriteTmprlData(char *name, int Iteration, double tc, double **xyDKO, int *JetRoots)
{
	...
	dx = xyDKO[0][JetRoots[1]] - xyDKO[0][JetRoots[0]];
	dy = xyDKO[1][JetRoots[1]] - xyDKO[1][JetRoots[0]];
	angle = 180.0 - atan2(dy, dx) * 180.0 / 3.1415926535897932384626433832795;
	if (angle < 0.0)
		angle += 360.0;
